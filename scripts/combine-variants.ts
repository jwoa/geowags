...
const FINISH_SUFFIXES = [
  'brushed', 'chrome', 'copper', 'gold', 'gun-metal', 'gun-metal',
...
function parseMarkdownFile(filepath: string): ProductFile | null {
  ...
  try {
    // Simple YAML parser (basic - handles simple key-value pairs)
    ...
    while (i < lines.length) {
      ...
      if (value === '') {
        ...
        while (i < lines.length) {
          ...
          } else if (arrayLine.startsWith(key + ':')) {
            break;
          } else {
            break;
          }
...
async function combineVariants() {
  ...
  // Group by base name (skip size variants)
  ...
  for (const [baseName, variants] of Object.entries(groups)) {
    ...
    mainProduct.data.colors = colors;
